...
namespace VersSdk
{
    ...
    public class EnvVarsResponse
    {
        ...
        public Dictionary<string, string> Vars { get; set; } = default!;
    }
    ...
    public class WhoamiApiKey
    {
        ...
        public long? MaxMemoryMib { get; set; }
        [JsonPropertyName("max_vcpus")]
        public int? MaxVcpus { get; set; }
    }

    /// <summary>Request body for POST /vm/{vm_id}/commit</summary>
    public class CommitVmRequest
    {
    ...
    public class CommitInfo
    {
        ...
        public string? GrandparentCommitId { get; set; }
        [JsonPropertyName("is_public")]
        public bool IsPublic { get; set; } = default!;
        [JsonPropertyName("name")]
        public string Name { get; set; } = default!;
    ...
    /// <summary>Request body for `PATCH /vm/{vm_id}/label` (upsert) and `PUT /vm/{vm_id}/labels` (replace).  `labels` is **required** on both endpoints. Sending a body without the field deserializes to a 400 — this is deliberate: the previous `Option<HashMap<…>>` shape silently no-op'd PATCH and (worse) silently wiped every label on PUT. An empty map (`{ 'labels': {} }`) is still accepted as an explicit 'clear all' for PUT and an explicit 'no-op' for PATCH.</summary>
    ...
    public class VmMetadataResponse
    {
        ...
        public string? GrandparentVmId { get; set; }
        [JsonPropertyName("ip")]
        public string Ip { get; set; } = default!;
        /// <summary>RAM in MiB allocated to this VM at create-time.</summary>
        [JsonPropertyName("mem_size_mib")]
        public int MemSizeMib { get; set; } = default!;
        [JsonPropertyName("owner_id")]
        public string OwnerId { get; set; } = default!;
        ...
        /// <summary>vCPUs allocated to this VM at create-time.</summary>
        [JsonPropertyName("vcpu_count")]
        public int VcpuCount { get; set; } = default!;
        [JsonPropertyName("vm_id")]
        public string VmId { get; set; } = default!;
    }

    /// <summary>Response body for `GET /api/v1/whoami`.  Identifies the principal (user + organization) behind the bearer token used for the request, plus minimal metadata about the API key itself.  The endpoint is read-only and requires only a valid API key — useful for answering questions like 'whose account does this key belong to?' without needing direct database access. Modeled after GitHub's `GET /user`.</summary>
    public class WhoamiResponse
    {
        [JsonPropertyName("api_key")]
        public WhoamiApiKey ApiKey { get; set; } = default!;
        [JsonPropertyName("org")]
        public WhoamiOrg Org { get; set; } = default!;
        [JsonPropertyName("user")]
        public WhoamiUser User { get; set; } = default!;
    }
